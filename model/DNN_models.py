...
def conv_autoencoder(dims, act='relu', init='glorot_uniform', latent_act=False, output_act=False, rf_rate=0.1, st_rate=0.25):
    ...
    for i in range(n_internal_layers):
        ...
        stride_size = int(rf_size / 2.0) if int(rf_size / 2.0) > 0 else 1
    ...
    # The original Deepmicro does not respect l_act=False, but instead had:
    # h = Flatten()(h)      TK
    ...
    # internal layers in decoder
    # Before l_act correction, loop was over range(n_internal_layers-1, 0, -1)
    for i in range(n_internal_layers, 0, -1):
